...
namespace GenTools.Depend
{
    internal class TerminalMode
    {
        ...
        public async Task<bool> Init(int Mode = 0, int SafeMode = 0, String PanicMessage = "Null",String OtherMessage = null)
        {
            ...
            Console.Title = "GenTools TerminalMode";
            Console.Clear();
            if (SafeMode == 0) { 
                if (Mode == 1)
                {
                    var list = new[] { "选择游戏路径", "抽卡分析", "设置", "[red]退出控制台模式[/]", "[bold red]退出GenTools[/]", };
                    if (localSettings.Values.ContainsKey("Config_GamePath"))
                    {
                        ...
                        {
                            list = new[] { "选择游戏路径", "[Cyan]显示主界面[/]", "[red]退出控制台模式[/]", "[bold red]退出GenTools[/]", };
                        }
                        else
                        {
                            list = new[] { "[bold green]开启游戏(120FPS)[/]", "[bold yellow]清除游戏路径[/]", "[Cyan]显示主界面[/]", "[red]退出控制台模式[/]", "[bold red]退出GenTools[/]" };
                        }
                    }
                    else
                    {
                    ...
                    var select = AnsiConsole.Prompt(
                            new SelectionPrompt<string>()
                                .Title("[bold green]GenTools[/] 控制台模式")
                                .PageSize(10)
                                .AddChoices(list));
            ...
            {
                Console.Title = "GenTools SafeMode";
                Console.Clear();
                Logging.Write("[red]错误问题：[/]" + PanicMessage,2);
                Logging.Write("其他信息：" + OtherMessage,2);
                var list = new[] { "检查本地设置参数", "[red]清空所有配置文件[/]", "[bold red]退出GenTools[/]" };
                var select = AnsiConsole.Prompt(
                        new SelectionPrompt<string>()
                            .Title("\n[bold red]GenTools 安全模式[/]")
                            .PageSize(10)
                            .AddChoices(list));
                switch (select)
                {
