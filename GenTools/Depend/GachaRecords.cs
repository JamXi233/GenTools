...
namespace GenTools.Depend
{
    public class GachaRecords
    {
        ...
        public async Task<List<GachaRecords>> GetAllGachaRecordsAsync(String url, String localData = null , String gachaType = "301")
        {
            ...
            if (localData == null){
                ...
                while (true)
                {
                    try
                    {
                        ...
                        var newurl = url.Substring(0, urlindex) + "&gacha_type=" + gachaType + "&size=10&end_id=" + endId;
                        var response = await client.GetAsync(newurl);
                        if (!response.IsSuccessStatusCode) break;
                        var json = await response.Content.ReadAsStringAsync();
                        var data = System.Text.Json.JsonSerializer.Deserialize<JsonElement>(json).GetProperty("data");
                        Logging.Write(url.Substring(0, urlindex) + "&gacha_type=" + gachaType + "&size=10&end_id=" + endId, 0);
                        Trace.WriteLine(url.Substring(0, urlindex) + "&gacha_type=" + gachaType + "&size=10&end_id=" + endId);
                        ...
                        else
                        {
                            ...
                            foreach (var item in data.GetProperty("list").EnumerateArray())
                            {
                                ...
                                Logging.Write(item.GetProperty("uid").GetString() + "|" + item.GetProperty("time").GetString() + "|" + item.GetProperty("id").GetString() + "|" + item.GetProperty("name").GetString() + "|" + item.GetProperty("id").GetString(), 0);
                                Trace.WriteLine(item.GetProperty("uid").GetString() + "|" + item.GetProperty("time").GetString() + "|" + item.GetProperty("id").GetString() + "|" + item.GetProperty("name").GetString() + "|" + item.GetProperty("id").GetString());
                            }
