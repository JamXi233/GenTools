...
namespace GenTools.Views
{
    public sealed partial class GachaView : Page
    {
        ...
        public GachaView()
        {
            ...
            string FileFolder = "\\JSG-LLC\\GenTools\\GachaRecords_Character.ini";
            string FileFolder2 = "\\JSG-LLC\\GenTools\\GachaRecords_Weapon.ini";
            string FileFolder3 = "\\JSG-LLC\\GenTools\\GachaRecords_Newbie.ini";
            string FileFolder4 = "\\JSG-LLC\\GenTools\\GachaRecords_Regular.ini";
            if (localSettings.Values["Gacha_Data"] as string == "1") 
            {
                ...
                { 
                    gacha_status.Text = "自上一次抽卡记录保存，文件可能手动变动，请重新获取";
            ...
            if (ProxyButton.IsChecked ?? false)
            {
                if (Process.GetProcessesByName("YuanShen").Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        string folderPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "JSG-LLC\\GenTools");
        ...
        {
            var char_records = await new GachaRecords().GetAllGachaRecordsAsync(url, null,"301");
            var light_records = await new GachaRecords().GetAllGachaRecordsAsync(url, null,"302");
            var newbie_records = await new GachaRecords().GetAllGachaRecordsAsync(url, null, "100");
            var regular_records = await new GachaRecords().GetAllGachaRecordsAsync(url, null, "200");
            var folder = KnownFolders.DocumentsLibrary;
            var GenToolsFolder = await folder.CreateFolderAsync("JSG-LLC\\GenTools", CreationCollisionOption.OpenIfExists);
            var char_gachaFile = await GenToolsFolder.CreateFileAsync("GachaRecords_Character.ini", CreationCollisionOption.OpenIfExists);
            var light_gachaFile = await GenToolsFolder.CreateFileAsync("GachaRecords_Weapon.ini", CreationCollisionOption.OpenIfExists);
            var newbie_gachaFile = await GenToolsFolder.CreateFileAsync("GachaRecords_Newbie.ini", CreationCollisionOption.OpenIfExists);
            var regular_gachaFile = await GenToolsFolder.CreateFileAsync("GachaRecords_Regular.ini", CreationCollisionOption.OpenIfExists);
            ...
            if (args.SelectedItemContainer != null)
            {
                switch (args.SelectedItemContainer.Tag.ToString())
                {
                    case "CharacterGacha":
                        ...
                        break;
                    case "LightConeGacha":
                        gachaFrame.Navigate(typeof(WeaponGachaView));
                        break;
                    case "RegularGacha":
