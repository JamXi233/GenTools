...
namespace GenTools.Views
{
    public sealed partial class AboutView : Page
    {
        ...
        public void Clear_AllData(object sender, RoutedEventArgs e)
        {
            ...
            DeleteFolder(userDocumentsFolderPath + "\\JSG-LLC\\GenTools\\", "1");
        }
        ...
        {
            int result = await OnGetUpdateLatestReleaseInfo("GenTools");
            if (result == 0)
            {
        ...
        {
            int result = await OnGetUpdateLatestReleaseInfo("GenToolsHelper","Depend");
            if (result == 0)
            {
        ...
        public async Task<int> OnGetUpdateLatestReleaseInfo(String PkgName,String Mode = null)
        {
            ...
            {
                ApplicationDataContainer localSettings = ApplicationData.Current.LocalSettings;
                var latestReleaseInfo = await _getJSGLatest.GetLatestReleaseInfoAsync("cn.jamsg.GenTools");
                ...
                {
                    string userDocumentsFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                    string path = userDocumentsFolderPath+ "\\JSG-LLC\\GenTools\\Depends\\GenToolsHelper\\GenToolsHelperVersion.ini";
        ...
        private async void UpdateDownload_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (localSettings.Values["Config_UpdateService"])
            {
                ...
                case 1:
                    Logging.Write("UService:Gitee", 0);
                    latestReleaseInfo = await _getGiteeLatest.GetLatestReleaseInfoAsync("JSG-JamXi", "GenTools");
                    break;
                case 2:
                    Logging.Write("UService:JSG-DS", 0);
                    latestReleaseInfo = await _getJSGLatest.GetLatestReleaseInfoAsync("cn.jamsg.GenTools");
                    break;
                default:
                    Logging.Write($"Invalid update service value: {localSettings.Values["Config_UpdateService"]}", 0);
                    throw new InvalidOperationException($"Invalid update service value: {localSettings.Values["Config_UpdateService"]}");
        ...
        private async void DependUpdateDownload_Click(object sender, RoutedEventArgs e)
        {
            ...
            Trace.WriteLine(fileUrl);
            string UpdateFileFolder = "\\JSG-LLC\\GenTools\\Depends\\";
            string UpdateFileName = "GenToolsHelper_" + latestReleaseInfo.Version + ".zip";
            string UpdateExtractedFolder = "GenToolsHelper";
            string userDocumentsFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            DeleteFolder(userDocumentsFolderPath + "\\JSG-LLC\\GenTools\\Depends\\","0");
            string localFilePath = Path.Combine(userDocumentsFolderPath + UpdateFileFolder, UpdateFileName);
        ...
        private async void Backup_Data(object sender, RoutedEventArgs e)
        {
            ...
            savePicker.FileTypeChoices.Add("Zip Archive", new List<string>() { ".GenToolsBackup" });
            savePicker.SuggestedStartLocation = PickerLocationId.DocumentsLibrary;
            savePicker.SuggestedFileName = "GenTools_Backup_" + formattedDate;
            ...
            if (file != null)
            {
                string startPath = userDocumentsFolderPath + @"\JSG-LLC\GenTools";
        ...
        private async void Restore_Data(TeachingTip e, object o)
        {
            ...
            picker.FileTypeFilter.Add(".GenToolsBackup");
            var window = new Window();
            var hwnd = WinRT.Interop.WindowNative.GetWindowHandle(window);
            ...
            Task.Run(() => ZipFile.ExtractToDirectory(file.Path, userDocumentsFolderPath + "\\JSG-LLC\\GenTools\\")).Wait();
            Application.Current.Exit();
        }
